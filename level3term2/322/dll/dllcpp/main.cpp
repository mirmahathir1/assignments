...
#define HARD_CODED_PIGGY_ENABLE false
#define HARD_CODED_GENERATOR_POLYNOMIAL "1001"
#define HARD_CODED_CRC_LOG_FLAG true

#define ENABLE_MAHATHIR_LOG false
#define FILE_REDIRECTION false
#define FILE_NAME "case3.doc"
#define draft_enabled false
...
string generator_polynomial = "1011";
bool crc_log_flag;

//METHODS FOR CRC CALCULATION
string convertToBinaryString(long long int decimalNumber) {
    if (decimalNumber == 0)
        return "0";
...
long long int convertToDecimal(string binary_string){
    long long int returned_number = 0;

    for (int i=0; i<binary_string.length(); i++){

        if (binary_string.at(i)=='1')
            returned_number += 1 << (binary_string.length() - i - 1);
    }

    return returned_number;
}

string get_CRC(string dataword_string){
    int length_of_generator = generator_polynomial.length();
    long long int generator = convertToDecimal(generator_polynomial);

    long long int dataword_integer = convertToDecimal(dataword_string);

    long long int dividend = dataword_integer << (length_of_generator-1);

    int shift_amount = (int) ceill(log2l(dividend+1)) - length_of_generator;
    long long int subtracted_result;

    while ((dividend >= generator) || (shift_amount >= 0)){

        subtracted_result = (dividend >> shift_amount) ^ generator;
        dividend = (dividend & ((1 << shift_amount) - 1)) | (subtracted_result << shift_amount);

        shift_amount = (int) ceill(log2l(dividend + 1)) - length_of_generator;
    }

    return convertToBinaryString(dividend);
}


string get_binary_string_of_frame(struct frm frame){
    string seqnum_string = convertToBinaryString(frame.seqnum);
    string acknum_string = convertToBinaryString(frame.acknum);

    string payload_string = frame.payload;
    string payload_string_binary = "";

    payload_string = payload_string.substr(0,4);

    for(int i = 0 ; i < payload_string.length(); i++){
        payload_string_binary += convertToBinaryString((int)payload_string.at(i));
    }

    string total_string = payload_string_binary+seqnum_string+acknum_string;

    return total_string.substr(0,24);
}

int get_CRC_of_frame(int device, struct frm frame){


    string dataword = get_binary_string_of_frame(frame);
    //string dataword = payload_string_binary;

    string remainder = get_CRC(dataword);
    ...
    cout<<"input bit string= "<<dataword<<", generator= "<<generator_polynomial<<", remainder= "<<remainder<<endl;

    return convertToDecimal(remainder);
}


void main2(){
    //cout<<get_CRC_of_frame()
}
/*
...
void send_ack_to_layer_1(int device, int seq)
{
    ...
    ackframe.checksum = get_CRC_of_frame(device, ackframe);
...
void A_output(struct pkt packet)
{
    ...
    if(outstanding_ack_of_A)
    {
        ...
        sending_frame.checksum = get_CRC_of_frame(device_A, sending_frame);
...
void B_output(struct pkt packet)
{
    ...
    if(outstanding_ack_of_B)
    {
        ...
        //sending_frame.checksum = get_checksum(sending_frame);
        sending_frame.checksum = get_CRC_of_frame(device_B, sending_frame);
    ...
    else
    {
        ...
        string binary_frame = get_binary_string_of_frame(sending_frame);

        cout<<binary_frame<<endl;

        int sent_word = get_CRC_of_frame(sending_frame);

        cout<<sent_word<<endl;

        cout<< check_CRC_of_frame(sent_word) << endl;*/
...
void A_input(struct frm frame)
{
    ...
    if(frame.checksum != get_CRC_of_frame(device_A, frame))
    {
        ...
        else
        {
            ...
            negative_ack.checksum = get_CRC_of_frame(device_A, negative_ack);
...
void B_input(struct frm frame)
{
    ...
    if(frame.checksum!=get_CRC_of_frame(device_B, frame))
    {
        ...
        else
        {
            ...
            strcpy(negative_ack.payload, frame.payload);
            //negative_ack.checksum = get_checksum(negative_ack);
            negative_ack.checksum = get_CRC_of_frame(device_B, negative_ack);
...
int main()
{
    ...
    while (1)
    {
        ...
        time_prebuilt = eventptr->evtime; /* update time to next event time */
...
void init() /* initialize the simulator */
{
    ...
    if(HARD_CODED_PARAMETER_ACTIVATION)
    {
        ...
        generator_polynomial = HARD_CODED_GENERATOR_POLYNOMIAL;
    ...
    else
    {
        ...
        int option;
        scanf("%d",&option);
        piggy_backing_enable=option==1;

        printf("Enter generator polynomial: ");
        cin>> generator_polynomial;

        printf("Enter crc log flag [ 1 to enable, 0 for disable ]:");
        cin>> option;
        crc_log_flag= option==1;
    ...
    time_prebuilt = 0.0;              /* initialize time to 0.0 */
...
void starttimer(int AorB /* A or B is trying to start timer */, float increment)
{
    ...
    evptr = (struct event *)malloc(sizeof(struct event));
    evptr->evtime = time_prebuilt + increment;
